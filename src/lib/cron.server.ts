...
export async function sendAbandonedCartEmails(limit = 5) {
  ...
  for (const c of customers) {
    ...
    // Claim this exact cart version before SMTP. Do not reset newer carts on completion.
    ...
    // طلب مسجّل بعد آخر تعديل: المتابعة أصبحت مع الفريق، وليس دليلاً على شراء أو دفع.
    ...
    const rows = items
      .map(
        (i) =>
          `<tr><td style="padding:6px 0">${escapeHtml(i.name)} × ${i.quantity}</td><td>${fmtSyp(i.price * i.quantity)} ل.س</td></tr>`,
      )
      .join('');
    try {
  ...
  <p style="color:#71717a;font-size:12px">الأسعار تقديرية وتُؤكَّد على واتساب. لإيقاف هذه الرسائل عدّل تفضيلاتك من حسابك.</p>
      ...
      console.error('[cron] reminder failed or delivery uncertain; no automatic resend', c.id);
    }
  }
  return { sent };
}

...
// كل الأعسال بسعر واحد: الكيلو ٢٠٠٠ ⇒ نصف الكيلو ١٠٠٠.
// غبار الطلع: الكيلو ٣٠٠٠ ⇒ علبة المئة غرام ٣٠٠.
// القيمة الوسطى هي السعر الذي حوّلته خطوة سابقة، حتى يبقى السكربت صالحاً للإعادة.
const PRODUCT_PRICES = new Map([
  ['عسل حبة البركة', [150000, 1500, 1000]],
  ['عسل الدردار', [130000, 1300, 1000]],
  ['عسل الجيجان', [120000, 1200, 1000]],
  ['عسل القبار', [140000, 1400, 1000]],
  ['عسل الشوكيات السوري', [145000, 1450, 1000]],
  ['العكبر (Propolis)', [90000, 900, 900]],
  ['غذاء ملكات النحل', [110000, 1100, 1100]],
  ['غبار الطلع', [80000, 800, 300]],
]);
let products = 0;
for (const [name, [old_, mid, to]] of PRODUCT_PRICES) {
  const res = await db.product.updateMany({
    where: { name, price: { in: [old_, mid] } },
    data: { price: to },
  });
  products += res.count;
}
